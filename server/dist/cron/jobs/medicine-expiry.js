...
export async function runMedicineExpiryCheck() {
    ...
    for (const med of expiringMedicines) {
        if (!orgMap.has(med.organizationId))
            orgMap.set(med.organizationId, []);
    ...
    if (expiredMedicines.length > 0) {
        const expiredOrgMap = new Map();
        for (const med of expiredMedicines) {
            if (!expiredOrgMap.has(med.organizationId))
                expiredOrgMap.set(med.organizationId, []);
...
//# sourceMappingURL=medicine-expiry.js.map