...
export async function generateInvoicePdf(invoiceId, organizationId) {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (org.phone || org.email)
            doc.text(`${org.phone || ""} ${org.email ? "| " + org.email : ""}`, { align: "center" });
        doc.moveDown(0.5);
        ...
        if (invoice.patient) {
            ...
            if (invoice.patient.patientId)
                doc.text(`ID: ${invoice.patient.patientId}`);
            if (invoice.patient.phone)
                doc.text(`Phone: ${invoice.patient.phone}`);
            if (invoice.patient.address)
                doc.text(invoice.patient.address);
            doc.moveDown(0.5);
        }
