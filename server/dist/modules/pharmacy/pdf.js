...
export async function generatePrescriptionPdf(prescriptionId, organizationId) {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (org?.phone)
            doc.fontSize(9).text(`Phone: ${org.phone}`, { align: "center" });
        if (org?.email)
            doc.fontSize(9).text(`Email: ${org.email}`, { align: "center" });
        doc.moveDown(0.5);
        ...
        if (patient.dob)
            doc.text(`DOB: ${new Date(patient.dob).toLocaleDateString("en-IN")}`);
        doc.moveDown(0.5);
        // Doctor info
        ...
        if (appointment?.token)
            doc.text(`Token: #${appointment.token}`);
        doc.moveDown(0.5);
        // Medicines table
