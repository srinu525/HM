...
export async function sendAppointmentReminder(to: string, patientName: string, doctorName: string, date: Date, token: number) {
  const html = `
...
export async function sendSubscriptionExpiry(to: string, orgName: string, planName: string, endDate: Date) {
  const html = `
...
export async function sendLowStockAlert(to: string, medicines: { name: string; stock: number }[]) {
